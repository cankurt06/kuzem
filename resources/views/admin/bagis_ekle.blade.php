...
@section('head')
    <title>{{config('app.name')}} | Admin Bağış Ekle</title>
    <link href="{{asset('admin/vendor/datatables/dataTables.bootstrap4.css')}}" rel="stylesheet">
    <link href="{{asset('admin/css/jquery-ui.min.css')}}" rel="stylesheet">
@stop
@section('content')
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800">Bağış Ekle</h1>
    <!-- DataTales Example -->
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <div class="row">
                <div class="col-xl-6"> <h6 class="m-2 font-weight-bold text-primary">Bağış Ekle</h6></div>
            </div>
        </div>
        <div class="card-body">
            <form autocomplete="off" id="bagis_ekle_form" action="{{route('bagis_ekle_post')}}" method="POST">
                @csrf

            <div class="form-group">
                <label for="bagis_adi" class="col-md-12 col-sm-12 col-xs-12">Bağış Adı <span class="required">*</span></label>
                <div class="col-md-8 col-sm-8 col-xs-12">
                    <input id="bagis_adi" class="form-control" required="required" name="bagis_adi" value="{{ old('bagis_adi') }}" type="text">
                </div>
            </div>
                <div class="form-group">
                    <label for="bagis_tarihi" class="col-md-12 col-sm-12 col-xs-12">Bağış Tarihi <span class="required">*</span></label>
                    <div class="col-md-8 col-sm-8 col-xs-12">
                        <input id="bagis_tarihi" class="form-control" required="required" name="bagis_tarihi" value="{{ old('bagis_tarihi') }}" type="text">
                    </div>
                </div>
            <div class="form-group">
                <label for="bagis_slogan" class="col-md-12 col-sm-12 col-xs-12">Bağış Slogan </label>
                <div class="col-md-8 col-sm-8 col-xs-12">
                    <input id="bagis_slogan" class="form-control" name="bagis_slogan" value="{{ old('bagis_slogan') }}" type="text">
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="col-md-8 col-sm-8 col-xs-12">
                  <select name="bagis_turu" id="bagis_turu" class="form-control">
                      @foreach(\App\BagisKategori::get() as $bagis_turu)
                          <option value="{{$bagis_turu->id}}">{{$bagis_turu->bagis_turu}}</option>
                          @endforeach
                  </select>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="col-md-8 col-sm-8 col-xs-12">
                    <input id="bagis_tutar" class="form-control" required="required" name="bagis_tutar" value="{{ old('bagis_tutari') }}" type="text">
                </div>
            </div>
            <div class="form-group">
                <label for="onemli_bagis" class="col-md-12 col-sm-12 col-xs-12">Önemli Bağış </label>
                <div class="col-md-8 col-sm-8 col-xs-12">
                    <input id="onemli_bagis"  name="onemli_bagis" type="checkbox"> Evet
                </div>
            </div>
            <div class="form-group">
                <label for="onemli_bagis" class="col-md-12 col-sm-12 col-xs-12">Bağış Detayları</label>
                <div class="col-md-12 col-xs-12">
                    <textarea name="bagis_icerik" id="bagis_icerik"></textarea>
                </div>
            </div>
            <div class="form-group">
                ...
                <div id="anasayfa_resim_div" onclick="window.open($('#anasayfa_resim_url').val())"></div>
                <input type="hidden" name="slayt_resmi" id="anasayfa_resim_url" value="{{old('slayt_resmi')}}"/>
            </div>
            <hr>
            <div class="form-group">
                ...
                <div id="haberler_resim_div" onclick="window.open($('#haberler_resim_url').val())"></div>
                <input type="hidden" name="bagis_resmi" id="haberler_resim_url" value="{{old('bagis_resmi')}}"/>
            </div>
