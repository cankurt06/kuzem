...
@section('head')
    <title>{{config('app.name')}} | Admin Haber Düzenle</title>
    <link href="{{asset('admin/vendor/datatables/dataTables.bootstrap4.css')}}" rel="stylesheet">
    <link href="{{asset('admin/css/jquery-ui.min.css')}}" rel="stylesheet">
@stop
@section('content')
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800">Haber Düzenle</h1>
    <!-- DataTales Example -->
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <div class="row">
                <div class="col-xl-6"> <h6 class="m-2 font-weight-bold text-primary">Haber Düzenle</h6></div>
            </div>
        </div>
        <div class="card-body">
            <form autocomplete="off" id="haber_duzenle_form" action="{{route('haber_duzenle_post')}}" method="POST">
                @csrf
            ...
            <div class="form-group">
                ...
                <div class="col-md-8 col-sm-8 col-xs-12">
                    <input id="haber_baslik" class="form-control" required="required" name="haber_baslik" value="{{ $haber->haber_baslik }}" type="text">
                </div>
            </div>
                <div class="form-group">
                    <label for="haber_tarihi" class="col-md-12 col-sm-12 col-xs-12">Haber Tarihi <span class="required">*</span></label>
                    <div class="col-md-8 col-sm-8 col-xs-12">
                        <input id="haber_tarihi" class="form-control" required="required" name="haber_tarihi" value="{{ carbon::parse($haber->created_at )->format('d.m.Y')}}" type="text">
                    </div>
                </div>
            <div class="form-group">
                <label for="haber_icerik" class="col-md-12 col-sm-12 col-xs-12">Haber İçeriği</label>
                <div class="col-md-12 col-xs-12">
                    <textarea name="haber_icerik" id="haber_icerik">{{ $haber->haber_icerik }}</textarea>
                </div>
            </div>
            <div class="form-group">
                ...
                <div id="anasayfa_resim_div" onclick="window.open($('#haber_resim_url').val())"><img src="{{asset($haber->haber_resim_url)}}" width="250px" style="margin-top: 15px"></div>
                <input type="hidden" name="haber_resim_url" id="haber_resim_url" value="{{ $haber->haber_resim_url}}"/>
            </div>
            <hr>
